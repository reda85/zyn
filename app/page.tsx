...
/* ── SETUP REQUIRED in layout.tsx:
   ...
   ENV URLs to replace (search for TODO_URL):
   - app.zaynspace.com/sign-up  → sign-up
   - cal.com/zaynspace/demo     → démo Calendly
*/
...
/* ────────────────────────────────────────────────────────────────
   ANIMATED PLAN MOCKUP (remplace l'ancien AppMockup + static img)
   Fidèle à l'app réelle: plan blanc technique, pins colorés avec
   icônes, FABs à droite (bleu/noir pulsant/violet)
   ──────────────────────────────────────────────────────────────── */

function PlanMockup() {
  const [replayKey, setReplayKey] = useState(0);

  // Chaque pin: position (x/y sur viewBox 960×600), couleur, icône, délai
  const pins = [
    { id: 1, x: 300, y: 85,  color: "#D946A8", Icon: PaintRoller, delay: 0.4, label: "Peinture à reprendre" },
    { id: 2, x: 380, y: 320, color: "#E11D48", Icon: Flame,       delay: 1.2, label: "Extincteur manquant" },
    { id: 3, x: 415, y: 340, color: "#D946A8", Icon: Lock,        delay: 2.0, label: "Serrure HS" },
    { id: 4, x: 150, y: 280, color: "#F59E0B", Icon: Zap,         delay: 2.8, label: "Prise cassée" },
    { id: 5, x: 560, y: 360, color: "#0EA5E9", Icon: Droplet,     delay: 3.6, label: "Fuite canalisation" },
  ];
  ...
  return (
    <div key={replayKey} className="relative w-full h-full">
      ...
      <div className="absolute inset-0 bg-white border border-slate-200 rounded-tl-xl rounded-tr-xl overflow-hidden shadow-[0_32px_80px_rgba(15,23,42,0.18)]">
        ...
        <div className="h-11 bg-slate-900 flex items-center px-4 gap-2 shrink-0 relative z-20">
          ...
          <div className="flex-1 mx-3 bg-white/10 rounded h-5 flex items-center px-2">
            <span className="text-white/40" style={{ fontSize: 9 }}>
              Résidence Almaz · Niveau R+2 · Casablanca
            </span>
          </div>
        ...
        <div className="relative h-[calc(100%-44px)] w-full overflow-hidden">
          ...
          >
            ...
            <g transform="translate(60, 60)">
              ...
              <g stroke="#0f172a" strokeWidth="3" fill="none" strokeLinecap="square">
                ...
                <line x1="380" y1="280" x2="640" y2="280" />
                <line x1="240" y1="200" x2="240" y2="420" />
              ...
              <g stroke="#475569" strokeWidth="0.8" opacity="0.4">
                {[0,1,2,3,4,5].map(i => (
                  <line key={i} x1={380 + i*4} y1={20} x2={372 + i*4} y2={280} />
                ))}
              </g>

              {/* Ouvertures (portes) */}
              <g stroke="white" strokeWidth="4">
                <line x1="130" y1="200" x2="180" y2="200" />
              ...
              <g fill="#64748b" fontSize="10" fontFamily="ui-sans-serif, system-ui" textAnchor="middle">
                <text x="200" y="12">10,63</text>
                <text x="510" y="12">8,42</text>
                <text x="130" y="448">6,25</text>
                <text x="370" y="448">0,32</text>
                <text x="570" y="448">5,18</text>
              </g>
              <g stroke="#94a3b8" strokeWidth="0.6" strokeDasharray="2,2">
                <line x1="20" y1="0" x2="380" y2="0" />
                <line x1="380" y1="0" x2="640" y2="0" />
                <line x1="20" y1="438" x2="640" y2="438" />
              </g>

              {/* Annotation A2 */}
              <g transform="translate(320, 240)">
                <polygon points="0,-8 -7,4 7,4" fill="#0f172a" />
              ...
              {/* Cote latérale */}
              <text x="670" y="220" fontSize="11" fill="#94a3b8" fontFamily="ui-sans-serif, system-ui" transform="rotate(-90, 670, 220)" textAnchor="middle">
                (38,80)
              </text>

              {/* Labels pièces */}
              <g fill="#cbd5e1" fontSize="9" fontFamily="ui-sans-serif, system-ui" textAnchor="middle" fontWeight="500" style={{ letterSpacing: "0.1em" }}>
                <text x="200" y="110">SÉJOUR</text>
                <text x="510" y="150">CHAMBRE 1</text>
                <text x="130" y="320">CUISINE</text>
                <text x="370" y="370">SDB</text>
                <text x="570" y="370">CHAMBRE 2</text>
              </g>
            </g>
              ...
              {pins.map(({ id, x, y, color, Icon, delay }) => (
                <g key={id} className="plan-pin" style={{ animationDelay: `${delay}s` }}>
                  ...
                  <circle
                    ...
                    fill={color}
                    opacity="0.25"
                    className="plan-pin-halo"
                    style={{ animationDelay: `${delay + 0.6}s`, transformOrigin: `${x}px ${y}px` }}
                  />
                  {/* Ring blanc */}
                  <circle cx={x} cy={y} r="17" fill="white" />
                  {/* Pin coloré */}
                  <circle cx={x} cy={y} r="15" fill={color} />
                  {/* Icône (via foreignObject pour utiliser Lucide) */}
            ...
            <g transform="translate(885, 420)">
              ...
              <foreignObject x="-10" y="-10" width="20" height="20">
                <div className="flex items-center justify-center w-full h-full gap-0.5">
                  <MapPin size={12} color="white" strokeWidth={2.2} fill="white" fillOpacity={0.2} />
                </div>
              </foreignObject>
              <foreignObject x="4" y="-14" width="14" height="14">
                <div className="flex items-center justify-center w-full h-full rounded-full bg-emerald-500">
            ...
            {/* FAB filtres (violet) */}
            <g transform="translate(885, 485)">
              <circle cx="0" cy="0" r="24" fill="#9333EA" />
          ...
          {/* Overlay bas: fondu pour blend avec la page */}
      ...
      <button
        ...
        className="absolute top-14 right-4 z-30 bg-white/95 backdrop-blur border border-slate-200 rounded-full px-3 py-1.5 flex items-center gap-1.5 text-[10px] font-medium text-slate-600 hover:text-slate-900 hover:border-slate-300 transition-all shadow-sm"
      >
...
export default function HomePage() {
  ...
  const signUpUrl =
    ...
    (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1")
      ? "http://app.localhost:3000/sign-up"
      : "https://app.zaynspace.com/sign-up";
  ...
  const demoUrl = "https://cal.com/zaynspace/demo"; // TODO_URL: remplace par ton vrai Cal.com
  ...
  const features = [
    ...
    { icon: ShieldCheck, title: "Protection juridique",    desc: "Chaque réserve est horodatée et documentée automatiquement. En cas de litige, votre historique complet est disponible, exportable en PDF légal." },
    { icon: FileText,    title: "Rapports OPR en un clic", desc: "Générez des rapports de levée de réserves professionnels en quelques secondes. Partagez-les directement avec vos clients et la maîtrise d'ouvrage." },
    { icon: MapPin,      title: "Géolocalisation précise", desc: "Placez chaque réserve sur le plan avec une précision chirurgicale. Les coordonnées GPS sont capturées automatiquement à chaque relevé terrain." },
    { icon: Camera,      title: "Documentation visuelle",  desc: "Associez photos et annotations directement aux réserves. Le contexte visuel est préservé avec la localisation exacte sur le plan." },
  ...
  const personas = [
    {
      ...
      tagline: "OPR et visites de chantier",
      points: [
        "Levées de réserves sur plan, depuis le chantier",
        "Rapports PDF brandés à votre agence",
        "Suivi des entreprises et sous-traitants",
      ],
    },
    {
      icon: Briefcase,
      title: "Pour les MOD / AMO",
      tagline: "Pilotage multi-chantiers",
      points: [
        "Tableau de bord consolidé de vos opérations",
        "Traçabilité complète pour la maîtrise d'ouvrage",
        "Rapports structurés dignes de vos honoraires",
      ],
    },
    {
      icon: HardHat,
      title: "Pour les promoteurs & BTP",
      tagline: "Qualité et non-conformités",
      points: [
        "Vue temps réel de l'avancement par lot",
        "Suivi des non-conformités par corps d'état",
        "Historique juridique opposable",
      ],
    },
  ];
  ...
  const pricingPlans = [
    {
      name: "Starter",
      price: "399",
      description: "Pour les architectes indépendants qui démarrent",
      features: [
    ...
    },
    {
      name: "Pro",
      price: "699",
      description: "Pour les petites agences et cabinets",
      features: [
        "À partir de 3 utilisateurs",
        "Projets illimités",
        "Rapports PDF brandés",
        "Stockage 50 Go",
        "Export Excel et partage client",
        "Support prioritaire",
      ],
      cta: "Commencer",
      highlight: true,
    },
    {
      name: "Business",
      price: "1 200",
      description: "Pour les MOD/AMO et structures multi-chantiers",
      features: [
    ...
    <div className="min-h-screen bg-[#fefefe] text-slate-900 font-outfit">
          ...
          <nav className="flex-1 flex flex-col justify-center gap-9">
            {[
              ...
              <a key={label} href={href} onClick={() => setMenuOpen(false)}
                className="font-outfit text-3xl font-medium text-slate-900 no-underline">
                {label}
              </a>
            ))}
          </nav>
      ...
      <section className="min-h-screen grid md:grid-cols-2 items-center pt-16 relative overflow-hidden">
        ...
        <div className="px-6 md:px-12 py-20 relative z-10">
          {/* Badge Morocco */}
          <div
            className="inline-flex items-center gap-2 bg-white border border-slate-200 rounded-full px-3 py-1.5 mb-6"
            style={{ opacity: 0, animation: "0.7s ease 0.1s forwards fadeOnly" }}
          >
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
            <span className="font-outfit text-[10px] font-medium text-slate-600 tracking-wide">
              Conçu pour les architectes & bureaux de contrôle au Maroc
            </span>
          </div>

          <h1 className="font-lexend font-bold text-slate-900 leading-[1.05] mb-1"
            style={{ fontSize: "clamp(36px,5vw,60px)", letterSpacing: "-0.02em", opacity: 0, animation: "0.9s cubic-bezier(0.16,1,0.3,1) 0.2s forwards slideUp" }}>
            Vos levées de réserves,
          </h1>
          <h1 className="font-lexend font-bold text-emerald-500 leading-[1.05] mb-7"
            style={{ fontSize: "clamp(36px,5vw,60px)", letterSpacing: "-0.02em", opacity: 0, animation: "0.9s cubic-bezier(0.16,1,0.3,1) 0.32s forwards slideUp" }}>
            finies sur le chantier.
          </h1>
          ...
          <p className="font-outfit text-base text-slate-500 leading-relaxed mb-10 max-w-md"
            ...
            ZaynSpace transforme vos visites de chantier en rapports PDF professionnels.
            Placez vos réserves sur plan, photographiez, générez — directement depuis votre mobile.
          </p>
        ...
        <div className="hidden md:flex flex-col justify-center relative h-screen bg-slate-50 border-l border-slate-100 overflow-hidden"
          ...
          {/* Floating card 1 — nouvelle réserve */}
          <div className="absolute top-24 right-6 bg-white rounded-xl shadow-xl border border-slate-100 p-3.5 w-48 z-20"
            style={{ animation: "float-card 4s ease-in-out infinite" }}>
            <div className="flex items-center gap-2 mb-2.5">
              <div className="w-2 h-2 rounded-full bg-rose-500" />
              <span className="font-outfit font-semibold text-[10px] text-rose-500 tracking-wide uppercase">Nouveau</span>
            </div>
          ...
          {/* Floating card 2 — terminé */}
          <div className="absolute top-56 left-4 bg-white rounded-xl shadow-xl border border-slate-100 p-3 w-40 z-20"
          ...
          {/* Floating card 3 — équipe */}
          <div className="absolute bottom-32 right-8 bg-slate-900 rounded-xl shadow-xl p-3 z-20 flex items-center gap-2.5"
            style={{ animation: "float-card 4s ease-in-out 2.5s infinite" }}>
            <div className="flex -space-x-1.5">
              {[["#10b981","M"],["#8b5cf6","S"],["#f59e0b","Y"]].map(([c,l]) => (
                <div key={l} className="w-6 h-6 rounded-full border-2 border-slate-900 flex items-center justify-center text-[8px] font-bold text-white font-outfit" style={{ background: c }}>
                  {l}
                </div>
              ))}
            </div>
        ...
        <div className="md:hidden px-6 pb-16">
          <div className="relative rounded-xl overflow-hidden shadow-xl border border-slate-200" style={{ aspectRatio: "4/5" }}>
            <PlanMockup />
          </div>
        </div>
      </section>

      {/* ============ TRUST BAR (remplace les fake stats) ============ */}
      <section className="border-t border-b border-slate-200 bg-white py-10 px-6">
        <FadeIn>
          <div className="max-w-5xl mx-auto text-center">
            <p className="font-outfit text-[10px] tracking-[0.2em] uppercase text-slate-400 mb-4">
              Actuellement en bêta avec les premières agences marocaines
            </p>
            <div className="flex flex-wrap justify-center items-center gap-x-10 gap-y-4 opacity-60">
              {/* Placeholders — remplace par vrais logos de Founding Customers */}
              {["Agence A", "Cabinet B", "MOD C", "Studio D", "Bureau E"].map(name => (
                <div key={name} className="font-outfit text-sm font-medium text-slate-400 tracking-wide">
                  {name}
                </div>
              ))}
            </div>
        ...
        <div className="max-w-5xl mx-auto">
          <FadeIn>
            <div className="text-center mb-16">
              ...
              <h2 className="font-lexend font-bold text-slate-900" style={{ fontSize: "clamp(28px,3.5vw,42px)", letterSpacing: "-0.015em" }}>
                Un outil, trois métiers du <span className="text-emerald-500">bâtiment.</span>
              </h2>
            </div>
          </FadeIn>
                    ...
                    <ul className="space-y-3">
                      {p.points.map(pt => (
                        <li key={pt} className="flex items-start gap-2.5">
                          ...
                          <span className="font-outfit text-sm text-slate-600 leading-snug">{pt}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
          ...
          <div className="hidden md:grid grid-cols-2 gap-20 items-start">
            ...
            <FadeIn delay={100}>
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-10 sticky top-20 shadow-sm min-h-64">
                ...
                <h3 className="font-lexend font-semibold text-slate-900 mb-3.5" style={{ fontSize: 22, letterSpacing: "-0.01em" }}>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <FadeIn>
            <div className="max-w-xl mx-auto mb-12 bg-slate-900 text-white rounded-lg p-4 flex items-center gap-3 shadow-lg">
              ...
              <p className="font-outfit text-[11px] md:text-sm leading-snug">
                <span className="font-semibold">Offre Founding Customer</span> : <span className="text-slate-300">-40% à vie pour les 15 premiers clients</span>
              </p>
            </div>
          </FadeIn>
          ...
          <FadeIn>
            <div className="text-center mb-14">
              ...
              <h2 className="font-lexend font-bold text-slate-900" style={{ fontSize: "clamp(28px,3.5vw,42px)", letterSpacing: "-0.015em" }}>
                ...
                <span className="text-emerald-500">sans surprise.</span>
              </h2>
              <p className="font-outfit text-sm text-slate-500 mt-4 max-w-md mx-auto">
                Facturation mensuelle en dirhams. -20% en annuel. Essai gratuit 14 jours sans carte bancaire.
              </p>
            </div>
          </FadeIn>
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {pricingPlans.map((plan, i) => (
              <FadeIn key={plan.name} delay={i * 100}>
                <div className={`h-full rounded-lg p-8 flex flex-col transition-all ${
                  plan.highlight
                    ? "bg-slate-900 text-white border border-slate-900 shadow-2xl md:scale-105"
                    : "bg-white border border-slate-200 hover:border-emerald-200"
                }`}>
                  {plan.highlight && (
                    <div className="mb-4 -mt-2">
                      <span className="inline-block bg-emerald-500 text-white text-[9px] font-semibold tracking-widest uppercase px-2.5 py-1 rounded">
                        Le plus populaire
                      </span>
                    </div>
                  )}
                  <h3 className={`font-lexend text-xl font-semibold mb-2 ${plan.highlight ? "text-white" : "text-slate-900"}`}>
                    {plan.name}
                  </h3>
                  <p className={`font-outfit text-sm mb-6 ${plan.highlight ? "text-slate-400" : "text-slate-500"}`}>
                    {plan.description}
                  </p>
                  <div className="mb-6">
                    <div className="flex items-baseline gap-1">
                      <span className={`font-lexend text-4xl font-bold ${plan.highlight ? "text-white" : "text-slate-900"}`}>
                        {plan.price}
                      </span>
                      <span className={`font-outfit text-sm ${plan.highlight ? "text-slate-400" : "text-slate-500"}`}>
                        MAD / mois
                      </span>
                    </div>
                    <div className={`font-outfit text-[10px] mt-1 ${plan.highlight ? "text-slate-500" : "text-slate-400"}`}>
                      par utilisateur, HT
                    </div>
                  </div>
                  <ul className="space-y-3 mb-8 flex-1">
                    {plan.features.map(f => (
                      <li key={f} className="flex items-start gap-2.5">
                        <CheckCircle2 size={14} className={`shrink-0 mt-0.5 ${plan.highlight ? "text-emerald-400" : "text-emerald-500"}`} />
                        <span className={`font-outfit text-sm leading-snug ${plan.highlight ? "text-slate-300" : "text-slate-600"}`}>
                          {f}
                        </span>
                      </li>
                    ))}
                  ...
                  <Link href={signUpUrl}
                    className={`font-outfit text-center text-[11px] tracking-widest uppercase font-medium py-3.5 rounded-sm transition-colors no-underline ${
                      plan.highlight
          ...
          <FadeIn delay={300}>
            <p className="text-center font-outfit text-[11px] text-slate-400 mt-10">
              Besoin d'un plan sur mesure pour une structure plus grande ? <a href={demoUrl} className="text-emerald-500 hover:text-emerald-600 underline underline-offset-2">Contactez-nous</a>.
            </p>
          </FadeIn>
        </div>
      </section>
        ...
        <div className="max-w-xl mx-auto">
          <FadeIn>
            <div className="text-center mb-16">
              ...
              <h2 className="font-lexend font-bold text-slate-900" style={{ fontSize: "clamp(26px,4vw,44px)", letterSpacing: "-0.015em" }}>
          ...
          {[
            ...
            { n:"02", title:"Placez vos réserves",   desc:"Sur le chantier, tapez sur le plan pour créer une réserve. Associez photos, localisation GPS, assignation et priorité." },
            { n:"03", title:"Invitez votre équipe",  desc:"Partagez le projet avec vos sous-traitants et clients. Les permissions sont granulaires, configurables par rôle." },
            { n:"04", title:"Générez vos rapports",  desc:"Tableau de bord en direct, alertes automatiques, rapports PDF générés à la demande en un clic — prêts à envoyer." },
          ].map(({ n, title, desc }, i) => (
      ...
      <section className="py-28 px-6 bg-slate-900 relative overflow-hidden">
        ...
        <div className="absolute top-1/2 left-1/2 w-[600px] h-[600px] -translate-x-1/2 -translate-y-1/2 pointer-events-none"
          style={{ background: "radial-gradient(ellipse,rgba(16,185,129,0.15) 0%,transparent 60%)" }} />

        <div className="max-w-xl mx-auto text-center relative z-10">
          <FadeIn>
            <div className="flex items-center gap-5 mb-12">
              <div className="flex-1 h-px bg-slate-700" />
              <span className="font-outfit text-[10px] font-medium tracking-[0.2em] uppercase text-emerald-400">Commencez dès aujourd'hui</span>
              <div className="flex-1 h-px bg-slate-700" />
            </div>
            <h2 className="font-lexend font-bold text-white leading-none mb-6"
              style={{ fontSize: "clamp(32px,6vw,64px)", letterSpacing: "-0.025em" }}>
              Prêt à finir vos OPR<br />
              <span className="text-emerald-400">sur le chantier ?</span>
            </h2>
            <div className="w-12 h-px mx-auto mb-7"
              style={{ background: "linear-gradient(90deg,transparent,#10b981 30%,#10b981 70%,transparent)" }} />
            <p className="font-outfit text-sm text-slate-400 leading-relaxed max-w-xs mx-auto mb-11">
              Rejoignez les architectes et bureaux de contrôle qui gagnent 3h par visite de chantier avec ZaynSpace.
            </p>
            <div className="flex gap-2.5 justify-center flex-wrap mb-6">
              <Link href={signUpUrl}
                className="font-outfit inline-flex items-center gap-2 text-[11px] tracking-widest uppercase font-medium bg-emerald-500 text-white px-10 py-4 rounded-sm hover:bg-emerald-600 transition-colors no-underline">
                Commencer gratuitement <ArrowUpRight size={14} />
              </Link>
              <Link href={demoUrl}
                className="font-outfit inline-flex items-center gap-2 text-[11px] tracking-widest uppercase text-white px-10 py-4 border border-slate-700 rounded-sm hover:border-slate-500 hover:bg-slate-800 transition-all no-underline">
                Réserver une démo
              </Link>
            </div>
            <p className="font-outfit text-[10px] text-slate-500 tracking-widest">
              Pas de carte bancaire · 14 jours offerts · Conforme loi 09-08
            </p>
          </FadeIn>
        </div>
      </section>
      ...
      <footer className="border-t border-slate-800 py-10 px-6 md:px-12 bg-slate-900">
        <div className="max-w-5xl mx-auto flex flex-wrap justify-between items-center gap-5">
          ...
          <div className="flex gap-7 flex-wrap">
            {[{ l:"Confidentialité",h:"/privacy"},{ l:"CGU",h:"/terms"},{ l:"Contact",h:"#"}].map(({ l, h }) => (
              <Link key={l} href={h}
                className="font-outfit text-[11px] tracking-widest uppercase text-slate-500 hover:text-slate-200 transition-colors no-underline">{l}</Link>
            ))}
          </div>
